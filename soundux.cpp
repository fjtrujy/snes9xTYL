...
static void S9xAPUSetEndOfSample (int i, Channel *);
static void S9xAPUSetEndX (int);
static void S9xSetEnvRate (Channel *, int32, int32);
static void MixStereo (int);
...
static void S9xAPUSetEndOfSample (int i, Channel *ch) {
...
static void S9xAPUSetEndX (int i) {
	if(!DoFakeMute) {
		APUPack.APU.DSP[APU_ENDX] |= 1 << i;
	}
}

static void S9xSetEnvRate (Channel *ch, int32 rate_count, int32 xtarget)
{
...
void S9xSetSoundVolume (int channel, short volume_left, short volume_right)
{
	...
	if (stSoundStatus.stereo_switch + 1)
	{
		volume_left  = ((stSoundStatus.stereo_switch & (  1 << channel)) ? volume_left  : 0);
		volume_right = ((stSoundStatus.stereo_switch & (256 << channel)) ? volume_right : 0);
	}
...
{
	SoundData.echo_buffer_size = (delay << 10) * stSoundStatus.playback_rate / 32000;
	/*if (!stSoundStatus.stereo)
...
void S9xFixSoundAfterSnapshotLoad (int version)
{
	...
	S9xSetEchoWriteEnable (!(APUPack.APU.DSP[APU_FLG] & APU_ECHO_DISABLED));
...
static void S9xSetSoundFrequency (int channel, int hertz)
{
	if (stSoundStatus.playback_rate)
		SoundData.channels[channel].frequency = (uint32)
			((int64) (hertz << (FIXED_POINT_SHIFT - 15)) * 32000 / stSoundStatus.playback_rate);
	if (Settings.FixFrequency)
		SoundData.channels[channel].frequency = (uint32)
			(SoundData.channels[channel].frequency * stSoundStatus.pitch_mul);
}
...
static void MixStereo (int sample_count)
{
	...
	for (uint32 J = 0; J < NUM_CHANNELS; J++)
	{
		...
		if (ch->state == SOUND_SILENT || !(stSoundStatus.sound_switch & (1 << J)))
			continue;

		bool8 mod1 = pitch_mod & (1 << J);
		bool8 mod2 = pitch_mod & (1 << (J + 1));
		bool8 env = !((ch->state == SOUND_SUSTAIN || ch->state == SOUND_GAIN ||
			ch->state == SOUND_DECREASE_EXPONENTIAL || ch->state == SOUND_DECREASE_LINEAR) 
			&& ch->xenv_rate == 0);
		bool8 echo = ch->echo_buf_ptr != 0;
...
void S9xResetSound (bool8 full)
{
	...
	if (stSoundStatus.playback_rate)
		stSoundStatus.err_rate = (uint32) (FIXED_POINT * SNES_SCANLINE_TIME * stSoundStatus.playback_rate);
	else
		stSoundStatus.err_rate = 0;
}
...
bool8 S9xInitSound (int mode, bool8 stereo, int buffer_size)
{
	...
	stSoundStatus.stereo = stereo;
	stSoundStatus.sixteen_bit = Settings.SixteenBitSound;
	stSoundStatus.encoded = FALSE;
	stSoundStatus.pitch_mul = 0.985; // XXX: necessary for most cards in linux...?
